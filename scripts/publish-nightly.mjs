 ...
 * publishes ONLY the core packages under the `nightly` dist-tag so `latest`
 * (the betas) is never moved. The version bump is left in the working tree
...
const filters = CORE_PACKAGES.map((name) => `--filter ${name}`).join(' ');
run(`pnpm ${filters} publish --no-git-checks --tag nightly${dryRun ? ' --dry-run' : ''}`);
