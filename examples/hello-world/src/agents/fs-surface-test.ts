...
const fsSurfaceTest = defineAction({
	name: 'fs-surface-test',
	description: 'Exercise the session.fs and harness.fs filesystem surfaces.',
	async run({ harness }) {
...
export default defineAgent(() => {
	...
	return {
		...
		instructions:
			'When asked to run a demo, call the `fs-surface-test` action and report its result.',
		actions: [fsSurfaceTest],
	};
});
