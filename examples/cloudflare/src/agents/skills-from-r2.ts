 ...
 * the model-callable `check_spam` action below (an action runs real code on a
...
export default defineAgent(async ({ env }) => {
	...
	return {
		...
		instructions:
			'When asked whether a message is spam, call the check_spam action with the message text and report its verdict.',
		actions: [checkSpam],
	};
});
