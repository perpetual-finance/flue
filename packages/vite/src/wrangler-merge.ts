 ...
 *
 * Ported from the CLI's build pipeline (packages/cli/src/lib/
 * cloudflare-wrangler-merge.ts, which keeps its own copy until Phase 6).
 *
 * Philosophy: the user's wrangler config is the source of truth. Flue
 * contributes the pieces it owns (the Worker entrypoint and its generated
 * per-agent Durable Object bindings) and leaves everything else untouched.
 * The merged result is written as `.flue-vite.wrangler.jsonc` and handed to
 * the sibling `@cloudflare/vite-plugin` as its input configuration.
 *
 * We delegate configuration parsing and validation to Wrangler, while
 * retaining environment blocks in the generated input configuration for the
 * Vite plugin. Flue owns Durable Object binding de-duplication and
 * Flue-specific validation (compat date floor, required compat flags), but
 * migration history remains entirely user-authored (adding an agent = file +
 * mount + migration tag).
 */
...
export async function readUserWranglerConfig(root: string): Promise<UserConfigRead> {
	...
	try {
		// Resolve Wrangler through the sibling `@cloudflare/vite-plugin` as seen
		// from the USER's project, so the config is parsed by the exact Wrangler
		// version that plugin drives. `@flue/vite` deliberately has no dependency
		// on either package (sibling contract). The plugin package is ESM-only
		// (no require-compatible export), so its directory is located by walking
		// the project's node_modules chain — the same walk npm/pnpm/yarn resolve
		// with — and Wrangler is then required from the real (symlink-resolved)
		// package location so pnpm's isolated layout finds the plugin's own
		// wrangler dependency.
		const pluginDir = resolvePackageDir(root, '@cloudflare/vite-plugin');
		if (!pluginDir) {
			throw new Error(`Cannot find @cloudflare/vite-plugin from ${root}.`);
		}
		const wranglerPath = createRequire(path.join(pluginDir, '__flue_vite_resolve__.cjs')).resolve(
			'wrangler',
		);
		...
		throw new Error(
			`[flue] Reading the Cloudflare wrangler config requires the Wrangler version provided by "@cloudflare/vite-plugin". ` +
				`Install @cloudflare/vite-plugin in this project.\n` +
...
/**
 * Locate a package's real directory as resolved from `root` by walking the
 * `node_modules` chain (symlinks resolved so pnpm's isolated layout exposes
 * the package's own dependencies to a `createRequire` anchored inside it).
 */
function resolvePackageDir(root: string, packageName: string): string | undefined {
 ...
 * config-resolution time is much friendlier.
 *
