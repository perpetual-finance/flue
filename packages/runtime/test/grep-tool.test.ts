...
import { createGlobTool, createGrepTool, overlayPackagedSkills } from '../src/agent.ts';
import { createNoopSessionEnv } from './fixtures/session-env.ts';

describe('standard shell tools', () => {
	...
	it('shares the backend probe across per-call overlay envs', async () => {
		const exec = vi
			.fn()
			.mockResolvedValueOnce({ stdout: 'ripgrep 14.1.0', stderr: '', exitCode: 0 })
			.mockResolvedValue({ stdout: 'src/a.ts:1:match', stderr: '', exitCode: 0 });
		...
		// The session hands tool factories a fresh overlay env per model call;
		// the probe must key on the stable exec reference, not env identity.
		await createGrepTool(overlayPackagedSkills(env, {})).execute('first', { pattern: 'match' });
		await createGrepTool(overlayPackagedSkills(env, {})).execute('second', { pattern: 'match' });

		expect(exec.mock.calls.filter(([cmd]) => cmd === 'rg --version')).toHaveLength(1);
	});
	...
	it('uses grep with extended regex when the rg probe is unavailable', async () => {
		...
		const grep = createGrepTool(createNoopSessionEnv({ exec }));
	...
	it('forwards the operation signal to env.exec when glob searches', async () => {
		...
		const glob = createGlobTool(createNoopSessionEnv({ exec }));
		const controller = new AbortController();
