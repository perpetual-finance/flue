...
- `dist/` — Vite's build output: `dist/server.mjs` (self-starting) plus `dist/app.mjs` (the non-listening application chunk `vite preview` serves) on the Node target, the Worker output on Cloudflare. Configure it with Vite's own `build.outDir` if needed.
