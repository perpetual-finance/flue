...
The tooling blueprint creates `createFlueAgentHarness(...)`, which opens a fresh conversation for each eval case (a new id appended to the agent's mount URL), prompts it through `@flue/sdk`, and records the agent's response, model usage, costs, and tool calls in the format expected by `vitest-evals`. A fresh conversation per case means saved conversation history cannot affect other cases.

For protected deployments, configure the harness with the required token or headers. See [Vitest Evals tooling](/docs/ecosystem/tooling/vitest-evals/) for setup and reporting details.
...
This case protects requirements that can be checked directly: the answer reports the current status, the agent consults the service-status tool, and the model reports token usage. Use direct assertions for requirements with a concrete expected result, and the harness's normalized transcript to inspect responses and tool use.
...
Start the application in one terminal (the blueprint pins a fixed dev port so the harness's default URL stays stable):
...
Set `FLUE_AGENT_URL` to the agent's mount URL on a deployed application to evaluate it instead of the local development server:

```sh
FLUE_AGENT_URL=https://your-app.example.com/agents/service-status pnpm run evals
```

The harness uses the agent's public HTTP route, so the agent under test must be mounted in `app.ts`. When evaluating a protected deployment, pass the required `token` or `headers` to `createFlueAgentHarness(...)`.
...
[Braintrust](https://www.braintrust.dev) provides a hosted platform for datasets, experiments, scoring, and comparing eval results over time. You can run Braintrust evals against the same public Flue agent APIs used in this guide. Flue also provides a [Braintrust tooling integration](/docs/ecosystem/tooling/braintrust/) that exports model, tool, task, usage, and error traces from the application. Experiment results and runtime traces serve different purposes, but identifiers such as `submissionId` and the conversation id can connect a failed case to the execution that produced it.

## Bring your own evals

You do not need `vitest-evals` to evaluate a Flue application. An eval can drive an agent with the SDK's conversation client — `send(...)` plus `wait(...)`, then read the reply from `history(...)` — compare against the behavior you expect, and send the score to any test runner or hosted platform. The harness in this guide packages that work for `vitest-evals`; use the same SDK boundary to integrate another eval library or your own scoring pipeline.
