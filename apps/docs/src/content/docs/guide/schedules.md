...
Schedules often start bounded work such as daily summaries, recurring reports, data synchronization, or cleanup. Model that work as an agent and deliver each occurrence with `dispatch(...)`. When the schedule's code needs the agent's result — to post it somewhere, or to gate a follow-up step — use `init(...).prompt(...)` instead on the Node target; on Cloudflare, deliver the input and let the agent publish its own result (see [Scripts](/docs/guide/scripts/#on-cloudflare)).
