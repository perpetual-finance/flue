...
export default defineAgent(() => ({
  model: 'anthropic/claude-sonnet-4-6',
  instructions: "Delegate MR reviews to the 'reviewer' subagent via a task.",
  subagents: [reviewer],
}));
```
...
export default defineAgent(() => ({
  sandbox: local(),
  model: 'anthropic/claude-sonnet-4-6',
  instructions: 'When given an issue IID, call the `triage-issue` action and report its result.',
  actions: [triage],
}));
```
