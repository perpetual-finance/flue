...
```ts
// Example: src/agents/triage.ts
'use agent';
import { defineAgent, useSandbox, useSkill, useTool } from '@flue/runtime';
import { local } from '@flue/runtime/node';

function Triage() {
	useTool(tool); // what it can do
	useSkill(skill); // expertise it can load on demand
	useSandbox(local()); // where it runs, safely
	return instructions; // who the agent is and how it works
}
...
function IssueTriage() {
	useTool(triageIssue); // an application-controlled capability the model can run
	return instructions;
}
const agent = defineAgent(IssueTriage, { model: 'anthropic/claude-sonnet-4-6' });

// Drive it in code: mount its routes, dispatch events into its
// conversations, or run it one-shot from the CLI.
await dispatch(agent, {
	id: issue.id,
	message: { kind: 'signal', type: 'issue.opened', body: issue.summary },
});
```
...
- [Why Flue](/docs/introduction/why-flue/) — the case for building agents this way.
- [Agents](/docs/guide/building-agents/) — defining an agent, its conversations, and its runtime configuration.
